...
class InventoryManagementPageAnalyzer:
    title = '肥料库存'
...
class InventoryManagementPage(AdvancedWorkbenchPage):
    table_name = 'inventory'
    title = '肥料库存'
    subtitle = '管理肥料、供应商、价格和库存，为方案落地提供约束。'
    filter_label = '类别'
    filter_field = 'category'
    action_label = '生成补货建议'
    fields = [
        {'name': 'material_name', 'label': '物料名称'},
        {'name': 'category', 'label': '类别', 'type': 'choice', 'choices': ['氮肥', '磷肥', '钾肥', '复合肥', '有机肥', '调理剂']},
        {'name': 'stock_kg', 'label': '库存(kg)', 'type': 'float', 'min': 0, 'max': 1000000},
        {'name': 'unit_price', 'label': '单价', 'type': 'float', 'min': 0, 'max': 1000000},
        {'name': 'supplier', 'label': '供应商'}
    ]
    search_fields = ['material_name', 'category', 'supplier']

    def __init__(self, repository: AppRepository, parent=None) -> None:
        self.analyzer = InventoryManagementPageAnalyzer()
    ...
    def _domain_primary_action(self) -> None:
        selected = self.context.selected
        if not selected:
            QMessageBox.information(self, '肥料库存', '请先选择库存记录。')
            return
        if float(selected.get('stock_kg', 0)) < 300:
            self.repository.create_record('tasks', {'task_name': '补充肥料库存', 'plot_name': '仓储', 'assignee': '待安排', 'due_date': self._future_day(2), 'progress': 0, 'status': '待安排'})
            QMessageBox.information(self, '已生成', '库存偏低，已生成补货任务。')
        else:
            QMessageBox.information(self, '库存充足', '当前物料库存暂不需要补货。')



class InventoryManagementPageRuleBook:
    def __init__(self) -> None:
        self.names = []
