...
class AlertCenterPage(AdvancedWorkbenchPage):
    table_name = 'alerts'
    title = '风险预警'
    subtitle = '记录养分异常、天气窗口、库存不足和执行延误等风险。'
    filter_label = '等级'
    filter_field = 'level'
    action_label = '生成处置任务'
    fields = [
        {'name': 'title', 'label': '标题'},
        {'name': 'level', 'label': '等级', 'type': 'choice', 'choices': ['低', '中', '高']},
        {'name': 'source', 'label': '来源', 'type': 'choice', 'choices': ['土壤', '气象', '库存', '任务', '算法']},
        {'name': 'content', 'label': '内容'},
        {'name': 'created_at', 'label': '发生日期', 'type': 'date'}
    ]
    search_fields = ['title', 'level', 'source', 'content']

    def __init__(self, repository: AppRepository, parent=None) -> None:
        self.analyzer = AlertCenterPageAnalyzer()
    ...
    def _domain_primary_action(self) -> None:
        selected = self.context.selected
        if not selected:
            QMessageBox.information(self, '风险预警', '请先选择预警。')
            return
        self.repository.create_record('tasks', {'task_name': '处置风险预警', 'plot_name': '待指定', 'assignee': '待安排', 'due_date': self._future_day(1), 'progress': 0, 'status': '待安排'})
        QMessageBox.information(self, '已生成', '已根据预警生成处置任务。')



class AlertCenterPageRuleBook:
    def __init__(self) -> None:
        self.names = []
