...
class TaskCenterPage(AdvancedWorkbenchPage):
    table_name = 'tasks'
    title = '作业任务'
    subtitle = '把决策方案拆成田间执行任务，跟踪进度与责任人。'
    filter_label = '状态'
    filter_field = 'status'
    action_label = '推进任务进度'
    fields = [
        ...
        {'name': 'assignee', 'label': '执行人'},
        {'name': 'due_date', 'label': '截止日期', 'type': 'date'},
        {'name': 'progress', 'label': '进度', 'type': 'int', 'min': 0, 'max': 100},
        {'name': 'status', 'label': '状态', 'type': 'choice', 'choices': ['备耕期', '生长期', '追肥期', '采收期', '休耕']}
    ]
    search_fields = ['task_name', 'plot_name', 'assignee', 'status']

    def __init__(self, repository: AppRepository, parent=None) -> None:
        self.analyzer = TaskCenterPageAnalyzer()
    ...
    def _domain_primary_action(self) -> None:
        selected = self.context.selected
        if not selected:
            QMessageBox.information(self, '作业任务', '请先选择任务。')
            return
        progress = min(100, int(selected.get('progress', 0)) + 15)
        status = '已完成' if progress >= 100 else '执行中'
        payload = dict(selected)
        payload['progress'] = progress
        payload['status'] = status
        self.repository.update_record('tasks', int(selected['id']), payload)
        QMessageBox.information(self, '已推进', f'任务进度已更新为{progress}%。')
        self.refresh()



class TaskCenterPageRuleBook:
    def __init__(self) -> None:
        self.names = []
