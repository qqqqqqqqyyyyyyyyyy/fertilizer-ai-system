...
class WeatherCenterPageAnalyzer:
    title = '气象水分'
...
class WeatherCenterPage(AdvancedWorkbenchPage):
    table_name = 'weather_profiles'
    title = '气象水分'
    subtitle = '维护降雨、温度、蒸散和灌溉条件，用于修正施肥时机与用量。'
    filter_label = '可灌溉'
    filter_field = 'irrigation_available'
    action_label = '生成天气窗口预警'
    fields = [
        {'name': 'station', 'label': '监测点'},
        {'name': 'rainfall_7d', 'label': '七日降雨', 'type': 'float', 'min': 0, 'max': 1000000},
        {'name': 'temperature_avg', 'label': '平均温度', 'type': 'float', 'min': 0, 'max': 1000000},
        {'name': 'evapotranspiration', 'label': '蒸散量', 'type': 'float', 'min': 0, 'max': 1000000},
        {'name': 'irrigation_available', 'label': '可灌溉', 'type': 'choice', 'choices': ['是', '否']},
        {'name': 'observed_at', 'label': '观测日期', 'type': 'date'}
    ]
    search_fields = ['station', 'irrigation_available', 'observed_at']

    def __init__(self, repository: AppRepository, parent=None) -> None:
        self.analyzer = WeatherCenterPageAnalyzer()
    ...
    def _domain_primary_action(self) -> None:
        selected = self.context.selected
        if not selected:
            QMessageBox.information(self, '气象水分', '请先选择气象记录。')
            return
        level = '高' if float(selected.get('rainfall_7d', 0)) > 60 or float(selected.get('temperature_avg', 0)) > 34 else '中'
        self.repository.create_record('alerts', {'title': '施肥天气窗口提醒', 'level': level, 'source': '气象', 'content': '根据当前气象水分记录生成作业窗口提醒。', 'created_at': self._today()})
        QMessageBox.information(self, '已生成', '已生成天气窗口预警。')



class WeatherCenterPageRuleBook:
    def __init__(self) -> None:
        self.names = []
