...
class ReportsCenterPageAnalyzer:
    title = '报表分析'
...
class ReportsCenterPage(AdvancedWorkbenchPage):
    table_name = 'fertilizer_decisions'
    title = '报表分析'
    subtitle = '汇总方案历史、风险分布、库存价值和待办执行情况。'
    filter_label = '作物'
    filter_field = 'crop'
    action_label = '生成报表任务'
    fields = [
    ...
    def __init__(self, repository: AppRepository, parent=None) -> None:
        self.analyzer = ReportsCenterPageAnalyzer()
    ...
    def _domain_primary_action(self) -> None:
        self.repository.create_record('tasks', {'task_name': '整理施肥分析报表', 'plot_name': '全局', 'assignee': '待安排', 'due_date': self._future_day(3), 'progress': 0, 'status': '待安排'})
        QMessageBox.information(self, '已生成', '已生成报表整理任务。')



class ReportsCenterPageRuleBook:
    def __init__(self) -> None:
        self.names = []
