...
class UserCenterPage(AdvancedWorkbenchPage):
    table_name = 'users'
    title = '用户与权限'
    subtitle = '管理业务用户、角色权限、登录审计和账号安全状态。'
    filter_label = '角色'
    filter_field = 'role'
    action_label = '新增业务用户'
    fields = [
        {'name': 'username', 'label': '用户名'},
        {'name': 'role', 'label': '角色', 'type': 'choice', 'choices': ['农技员', '管理员', '合作社负责人']}
    ]
    search_fields = ['username', 'role']

    def __init__(self, repository: AppRepository, current_user: dict | None = None, parent=None) -> None:
        self.current_user = current_user or {}
        self.analyzer = UserCenterPageAnalyzer()
    ...
    def refresh(self) -> None:
        self.records = self.repository.list_users()
    ...
    def edit_record(self) -> None:
        ...
        if not record:
            return
        dialog = RecordDialog("调整角色", [{"name": "role", "label": "角色", "type": "choice", "choices": ["农技员", "管理员", "合作社负责人"]}], {"role": record.get("role", "农技员")}, self)
        if dialog.exec():
            self.repository.update_user_role(int(record["id"]), dialog.payload()["role"])
            self.refresh()
    ...
    def delete_record(self) -> None:
        ...
        if not record:
            return
        try:
            self.repository.delete_user(int(record["id"]))
    ...
    def register_user(self) -> None:
        dialog = RecordDialog("新增业务用户", [{"name": "username", "label": "用户名"}, {"name": "password", "label": "密码"}, {"name": "role", "label": "角色", "type": "choice", "choices": ["农技员", "管理员", "合作社负责人"]}], parent=self)
        if dialog.exec():
            payload = dialog.payload()
            try:
                self.repository.register_user(payload["username"], payload["password"], payload["role"])
            except Exception as exc:
                QMessageBox.warning(self, "注册失败", str(exc))
                return
            QMessageBox.information(self, "注册成功", "新用户已经可以登录系统。")
            self.refresh()



class UserCenterPageRuleBook:
    def __init__(self) -> None:
        self.names = []
