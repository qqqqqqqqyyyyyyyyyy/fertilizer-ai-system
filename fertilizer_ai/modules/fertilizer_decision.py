...
class FertilizerDecisionPageAnalyzer:
    title = '施肥决策'
...
class FertilizerDecisionPage(AdvancedWorkbenchPage):
    table_name = 'fertilizer_decisions'
    title = '施肥决策'
    subtitle = '生成、保存和复盘氮磷钾及有机肥精准施用方案。'
    filter_label = '风险等级'
    ...
    fields = [
        ...
        {'name': 'nitrogen_kg', 'label': '氮肥(kg)', 'type': 'float', 'min': 0, 'max': 1000000},
        {'name': 'phosphorus_kg', 'label': '磷肥(kg)', 'type': 'float', 'min': 0, 'max': 1000000},
        {'name': 'potassium_kg', 'label': '钾肥(kg)', 'type': 'float', 'min': 0, 'max': 1000000},
        {'name': 'organic_kg', 'label': '有机肥(kg)', 'type': 'float', 'min': 0, 'max': 1000000},
        {'name': 'risk_level', 'label': '风险等级', 'type': 'choice', 'choices': ['低', '中', '高']},
        {'name': 'confidence', 'label': '可信度', 'type': 'float', 'min': 0, 'max': 1000000},
        {'name': 'notes', 'label': '说明'},
        {'name': 'created_at', 'label': '生成时间', 'type': 'date'}
    ]
    search_fields = ['plot_name', 'crop', 'risk_level', 'notes']

    def __init__(self, repository: AppRepository, parent=None) -> None:
        self.analyzer = FertilizerDecisionPageAnalyzer()
    ...
    def _domain_primary_action(self) -> None:
        samples = self.repository.list_records('soil_samples')
        weathers = self.repository.list_records('weather_profiles')
        if not samples or not weathers:
            QMessageBox.information(self, '数据不足', '请先录入土壤检测和气象水分数据。')
            return
        sample = samples[0]
        weather = weathers[0]
        model_sample = SoilSample(sample['plot_name'], sample['crop'], float(sample['area_mu']), float(sample['ph']), float(sample['organic_matter']), float(sample['nitrogen']), float(sample['phosphorus']), float(sample['potassium']), float(sample['moisture']), date.fromisoformat(sample['sampling_date']))
        model_weather = WeatherProfile(float(weather['rainfall_7d']), float(weather['temperature_avg']), float(weather['evapotranspiration']), weather['irrigation_available'] == '是')
        plan = self.analyzer.engine.build_plan(model_sample, model_weather)
        self.repository.save_decision({'plot_name': sample['plot_name'], 'crop': sample['crop'], 'nitrogen_kg': plan.nitrogen_kg, 'phosphorus_kg': plan.phosphorus_kg, 'potassium_kg': plan.potassium_kg, 'organic_kg': plan.organic_kg, 'risk_level': plan.risk_level, 'confidence': plan.confidence, 'notes': '；'.join(plan.notes)})
        QMessageBox.information(self, '已生成', '已根据最新土壤和气象数据重新生成施肥方案。')
        self.refresh()



class FertilizerDecisionPageRuleBook:
    def __init__(self) -> None:
        self.names = []
