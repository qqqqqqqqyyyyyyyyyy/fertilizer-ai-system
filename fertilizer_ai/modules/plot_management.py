...
class PlotManagementPageAnalyzer:
    title = '地块档案'
...
class PlotManagementPage(AdvancedWorkbenchPage):
    table_name = 'plots'
    title = '地块档案'
    subtitle = '维护种植区域、作物、面积、负责人和当前状态。'
    filter_label = '作物'
    filter_field = 'crop'
    action_label = '生成巡田任务'
    fields = [
        {'name': 'name', 'label': '地块名称'},
        ...
        {'name': 'status', 'label': '状态', 'type': 'choice', 'choices': ['备耕期', '生长期', '追肥期', '采收期', '休耕']}
    ]
    search_fields = ['name', 'crop', 'location', 'manager', 'status']

    def __init__(self, repository: AppRepository, parent=None) -> None:
        self.analyzer = PlotManagementPageAnalyzer()
    ...
    def _domain_primary_action(self) -> None:
        selected = self.context.selected
        if not selected:
            QMessageBox.information(self, '地块档案', '请先选择地块。')
            return
        self.repository.create_record('tasks', {'task_name': '巡田复核地块状态', 'plot_name': selected['name'], 'assignee': selected.get('manager','待安排'), 'due_date': self._future_day(1), 'progress': 0, 'status': '待安排'})
        QMessageBox.information(self, '已生成', '已为该地块生成巡田任务。')
        self.refresh()



class PlotManagementPageRuleBook:
    def __init__(self) -> None:
        self.names = []
